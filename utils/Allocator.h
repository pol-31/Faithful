...
struct SizeComparator {
  constexpr bool operator()(const std::pair<std::size_t, T>& arg1,
...
struct AllocatorTraits {
  ...
  static void Deallocate(AllocatorType &alloc, Pointer ptr,
                         SizeType size) {
...
//   free() all memory or __transfer__ it to another allocators (e.g. to G-P alloc)
...
// TODO: class BufferAllocator (storage buffers (256-1024 bytes) + smart reusing)
//      set of buffers?? (that is why we've implemented Span comparator)
//      __for_Buffer.h__
// allocate buffers separatedly and reuse them
template<typename T>
 class SpanBufferAllocator final
     : public details::allocator::BaseAllocator<T> {
 public:
  ...
  using SizeType = typename Base::SizeType;
   ...
   void Construct(T *ptr, Args... args) {
     AllocatorTraits<SpanBufferAllocator<T>>::Construct(
       *this, ptr, std::forward(args)...);
   }
...
template<typename T>
class RawBufferAllocator final
    : public details::allocator::BaseAllocator<T> {
 public:
  using Base = details::allocator::BaseAllocator<T>;
  using ValueType = typename Base::ValueType;
  ...
  static Pointer Allocate(SizeType size) {
    ...
    if (matched_size != buffer_pool_->end()) {
      auto new_buffer = std::move(*matched_size);
      buffer_pool_->erase(matched_size);
      allocated_->insert(std::make_pair(new_buffer.second,
                                        new_buffer.first));
      return new_buffer.second;
    }
    auto new_buffer = new T[size];
  ...
  static void Deallocate(Pointer ptr) {
    ...
    if (node != allocated_->end()) {
      buffer_pool_->insert(std::pair<std::size_t, Pointer>(node->second,
                                                           ptr));
...
template<typename T>
std::multiset<std::pair<std::size_t,
  typename BaseAllocator<T>::Pointer>,
  SizeComparator<typename BaseAllocator<T>::Pointer>>
  *BaseAllocator<T>::buffer_pool_ =
    new std::multiset<std::pair<std::size_t,
      typename BaseAllocator<T>::Pointer>,
      SizeComparator<typename BaseAllocator<T>::Pointer>>;
