...
class SingleSemiDeferredCall {
 public:
  ...
  SingleSemiDeferredCall(class Executor* executor) : executor_(executor) {}
  ...
  SingleSemiDeferredCall &Do(Callable &&fn) {
    ...
    executor_->Put([f = std::forward<Callable>(fn)]() { f(); });
    return *this;
  }
  ...
  SingleSemiDeferredCall &Do(Callable &&fn, ResType &result) {
    static_assert(std::is_invocable_v<Callable>,
      "Callable must be invocable.");
    executor_->Put(std::move(
      [this, f = std::forward<Callable>(fn), &result]() {
      &result = f();
    }));
    return *this;
  }
...
class ImmediateCall : public SingleSemiDeferredCall {
 public:
  ...
  ImmediateCall(class Executor* executor) : SingleSemiDeferredCall(executor) {}
  ...
  typename std::enable_if<std::is_void<
    decltype(std::declval<Callable>()())>::value, ImmediateCall &>::type
  Do(Callable &&fn) {
  ...
  Do(Callable &&fn) {
    static_assert(std::is_invocable_v<Callable>, "Callable must be invocable.");
    executor_->Put([this, f = std::forward<Callable>(fn)]() {
...
class DeferredCall : public SingleSemiDeferredCall {
 public:
  ...
  DeferredCall(class Executor* executor) : SingleSemiDeferredCall(executor) {}
  ...
  DeferredCall &Do(Callable &&fn, ResType &result) {
    ...
    if (continious_ == nullptr) {
      ...
      continious_ = new folly::Function<void(void)>(std::move(
        [this, f = std::forward<Callable>(fn), &result]() {
        &result = f();
      }));
    }
