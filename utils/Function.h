...
struct IsNullptrCompatible {
  template <typename U>
  static auto test(U* ptr) -> decltype(static_cast<bool>(static_cast<U const&>(*ptr) == nullptr), std::true_type{});
...
class Function final : private detail::function::FunctionTraits<FunctionType> {
  ...
  //mutable Data data_{unsafe_default_initialized}; ---------------------------------- r3
  Data data_{};
 ...
 public:
  ...
  template <
    typename Fun,
    typename =
    std::enable_if_t<!detail::function::is_similar_instantiation_v<Function, Fun>>,
...
class FunctionRef<ReturnType(Args...)> final {
 ...
 public:
  ...
  constexpr /* implicit */ FunctionRef(Fun&& fun) noexcept {
    auto& ref = fun;
    if constexpr (
      detail::function::IsNullptrCompatible<std::decay_t<Fun>>::value) {
      if (detail::function::isEmptyFunction(fun)) {
        return;
      }
    }
  ...
  template <
    ...
    std::enable_if_t<std::is_invocable_r_v<ReturnType, Fun&, Args&&...>, int> = 0>
  constexpr /* implicit */ FunctionRef(Fun* fun) noexcept {
    if (fun) {
