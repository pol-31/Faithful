...
void AudioProcessor::EncodeMusic(const std::filesystem::path& model_path,
  ...
  int buffer_size = frames_decompress_count_ * channels * 100; // average packet size TODO: too hard...
  ...
  if (model_path.extension() == ".mp3") {
    drmp3 drmp3_context;
    if (!drmp3_init_file(&drmp3_context, model_path.c_str(), nullptr)) { // TODO: allocation
...
//        out_audio.write(b.second, b.first);
    ...
    do {
      DecompressFlacChunk(model_path, &pPCM, &frames,
                          &channels, &sampleRate);
      if (frames == 0) break;
      PrepareEncodingContext(model_path, channels, sampleRate);
      CompressChunk(model_path, pPCM, frames, channels, sampleRate, buffers);
    ...
    do {
      DecompressOggChunk(model_path, &pPCM, &frames,
                         &channels, &sampleRate);
      if (frames == 0) break;
      PrepareEncodingContext(model_path, channels, sampleRate);
      CompressChunk(model_path, pPCM, frames, channels, sampleRate, buffers);
...
void AudioProcessor::DecompressMp3Chunk(drmp3& drmp3_context, int channels,
  ...
  size_t buffer_size = frames_decompress_count_ * channels * 800;// / 100; // TODO: ............
...
void AudioProcessor::DecompressFlacChunk(
  ...
  *frames = 0;
  size_t framesRead;
  while ((framesRead = drflac_read_pcm_frames_f32(pFlac, frames_decompress_count_, *pPCM + (*frames * (*channels)))) > 0) {
    *frames += framesRead;
...
void AudioProcessor::DecompressWavChunk(
    const std::filesystem::path& model_path, float** pPCM, uint64_t* frames,
    int* channels, int* sampleRate) {
  drwav wav;
  ...
  *frames = 0;
  size_t framesRead;
  while ((framesRead = drwav_read_pcm_frames_f32(&wav, frames_decompress_count_, *pPCM + (*frames * (*channels)))) > 0) {
    *frames += framesRead;
    size_t newSize = (*frames + frames_decompress_count_) * (*channels);
    if (newSize > initialBufferSize) {
      *pPCM = (float*)realloc(*pPCM, sizeof(float) * newSize);
...
void AudioProcessor::CompressChunk(const std::filesystem::path& model_path,
  ...
  for (int k = 0; k < thread_data_->size() - 1; ++k) {
    thread_pool_->threads_tasks_[k].first = std::move(
      [&, k, qwerty, thread_offset]() {
        ...
        while (true) {
          cur_pos = next_pos;
          next_pos += qwerty *
                      channels; // TODO: instead of checking every time precalculate all pair<start, len>
          ...
          for (int j = 0; j < channels; ++j) {
            for (uint64_t i = 0; i < sentinel; ++i) {
              buffer[j][i] = pPCM[cur_pos + i * channels + j]; // TODO: thread distinct offset--------------------------------------
            }
          }
          ...
          while (vorbis_analysis_blockout(&context.vd, &context.vb) == 1) {
            ...
            while (ogg_stream_pageout(&context.os, &context.og)) {
              std::memcpy(
                reinterpret_cast<void *>(buffers[k].second/* + buffers[k].first*/),
  ...
  while (true) {
    cur_pos = next_pos;
    next_pos += qwerty * channels; // TODO: instead of checking every time precalculate all pair<start, len>
...
//    std::ofstream out_audio("audio_result.ogg", std::ios::out | std::ios::app);
    ...
    while (vorbis_analysis_blockout(&context.vd, &context.vb) == 1) {
      ...
      ogg_stream_packetin(&context.os, &context.op);
      while (ogg_stream_pageout(&context.os, &context.og)) {
        if (buffers[context_idx].first < 0) break;
        std::cout << "buffer " << buffers[context_idx].first << std::endl;
//        out_audio.write(reinterpret_cast<const char*>(context.og.header), context.og.header_len);
//        out_audio.write(reinterpret_cast<const char*>(context.og.body), context.og.body_len);
        std::memcpy(reinterpret_cast<void*>(buffers[context_idx].second + buffers[context_idx].first),
                    reinterpret_cast<void*>(context.og.header),
