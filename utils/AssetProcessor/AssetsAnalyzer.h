...
class AssetsAnalyzer {
 public:
  AssetsAnalyzer(AssetLoadingThreadPool& thread_pool, bool force = false)
    : thread_pool_(thread_pool), force_(force) {}
  ...
  void GatherEncodeThreadInfo(
    ...
    const std::filesystem::path& path3 = FAITHFUL_ASSET_TEXTURE_PATH); // TODO: rename
 ...
 private:
  ...
  bool AreAssetsEqual(const AssetInfo& new_asset,
