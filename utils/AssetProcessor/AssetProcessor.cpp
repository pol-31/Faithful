...
void AssetProcessor::ProcessEncoding(const std::filesystem::path& path) {
  ...
  AudioProcessor audio_processor(true, asset_destination_ / "audio", thread_pool_);
  TextureProcessor texture_processor(true, asset_destination_ / "textures", thread_pool_);
  /*
   *
    bool encode,
  ...
  ModelProcessor model_processor(true, &assets_info, path, asset_destination_ / "models");
  ...
  while (true) {
    ...
    if (!thread_pool_->single_threaded_tasks_.empty()) {
      ...
      thread_pool_->single_threaded_tasks_accessor_.clear(); // TODO: memory order
      break;
    }
    thread_pool_->single_threaded_tasks_accessor_.clear(); // TODO: memory order
  }
...
void AssetProcessor::ProcessDecoding(const std::filesystem::path& path) { // path is destination folder
  AssetsAnalyzer assets_info(*thread_pool_);
