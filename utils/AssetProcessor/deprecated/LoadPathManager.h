...
class LoadPathManager {
 protected:
  ...
  static T* Find(const char* path) {
    ...
    for (auto entry = founded_file.first; entry != founded_file.second;
         ++entry) {
      for (auto it1 = entry->first.cbegin(), it2 = new_file.cbegin();
           it1 != entry->first.cend(); ++it1, ++it2) {
        if (*it1 != *it2) {
          found = false;
          break;
        }
...
template <typename T>
std::multimap<typename LoadPathManager<T>::FileNameType, T,
              typename LoadPathManager<T>::FileNameCmpType>*
    LoadPathManager<T>::loaded_files_ =
        new std::multimap<LoadPathManager<T>::FileNameType, T,
