...
AnimationNode::AnimationNode(const aiNodeAnim* node, glm::mat4 bone_transform,
                             ...
                             std::size_t children_num) : bone_id_(bone_id) {
...
void AnimationNode::InsertNode(std::size_t child_num, AnimationNode* child_node) {
...
void AnimationNode::ProcessAnimationKey(
  ...
  if (!keys_.Empty()) {
    glm::mat4 animation_s = glm::scale(glm::mat4(1.0f), AnimScalingByTime(time));
    glm::mat4 animation_r = glm::mat4_cast(AnimRotationByTime(time));
    glm::mat4 animation_t = glm::translate(glm::mat4(1.0f), AnimPositionByTime(time));
  ...
  out_bone_transform[bone_id_] = /*global_inverse_transform_ **/ cur_transform * (*bone_transform_);
  for (int i = 0; i < children_.get_size(); ++i) {
    children_[i]->ProcessAnimationKey(time, cur_transform, out_bone_transform, global_inverse_transform_);
  }
}
...
AnimationNode::AnimTransform AnimationNode::AnimTransformByTime(
  ...
  SkipIterator<float> begin(&*(keys_.begin() + global_offset + local_size - 1), local_size);
  /// last is not included (by std::lower_bound definition)
  // TODO: "- local_size"
  SkipIterator<float> end(&*(keys_.begin() + global_offset + global_size - local_size - 1), local_size);
  ...
  if ((resultIterator == begin) || ((resultIterator == end) && (*resultIterator < time))) {
    unsigned int offset = std::distance(keys_.get_data(), &*end) - local_size + 1;
    // TODO: we don't need result continiously (every frame) but sometimes only once
    return {offset, offset, 0};
  }
  unsigned int offset_last = std::distance(keys_.get_data(), &*resultIterator) - local_size + 1;
...
glm::vec3 AnimationNode::AnimPositionByTime(float time) {
  ...
  AnimTransform frame_info = AnimTransformByTime(get_position_offset(),
                                                 size, 4, time);
  if (frame_info.factor == 0)
    return {
      keys_[frame_info.idx_first],
  ...
  glm::vec3 end = {
    keys_[frame_info.idx_last],
    keys_[frame_info.idx_last + 1],
...
glm::quat AnimationNode::AnimRotationByTime(float time) {
  ...
  AnimTransform frame_info = AnimTransformByTime(get_rotation_offset(),
                                                 size, 5, time);
  if (frame_info.factor == 0)
    return {
      keys_[frame_info.idx_first],
...
glm::vec3 AnimationNode::AnimScalingByTime(float time) {
  ...
  glm::vec3 start = {keys_[frame_info.idx_first],
                     keys_[frame_info.idx_first + 1],
                     keys_[frame_info.idx_first + 2]};
  glm::vec3 end = {keys_[frame_info.idx_last],
                   keys_[frame_info.idx_last + 1],
                   keys_[frame_info.idx_last + 2]};
...
void AnimationNode::AssimpNodeAnimToSimurghAnimationNode(const aiNodeAnim* node) {
  if (node == nullptr) return;
  keys_ = utility::Span<float>(4 * (node->mNumScalingKeys + node->mNumPositionKeys)
