...
Mesh::Mesh(std::vector<Vertex> &&vertices, std::vector<unsigned int> &&indices,
  ...
  /*
  if (!static_load) {
    ImmediateCall{}.Do([=, vertices = std::move(vertices), indices = std::move(indices)]() mutable {
      this->Setup(vertices, indices, vao, vbo, ebo);
    });
  } else {
    Setup(std::move(vertices), std::move(indices), vao, vbo, ebo);
  }*/
...
void Mesh::Setup(std::vector<Vertex> vertices, std::vector<unsigned int> indices, GLuint vao, GLuint vbo, GLuint ebo) {
...
void SkinnedMesh::Setup(std::vector<SkinnedVertex> vertices,
  ...
  glEnableVertexAttribArray(1);
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(SkinnedVertex), (void *) offsetof(SkinnedVertex, tex_coords));
  ...
  glVertexAttribPointer(5, 4, GL_FLOAT, GL_FALSE, sizeof(SkinnedVertex), (void *) offsetof(SkinnedVertex, bone_weights));

  glEnableVertexAttribArray(6);
  glVertexAttribIPointer(6, 4, GL_UNSIGNED_INT, sizeof(SkinnedVertex), (void *) offsetof(SkinnedVertex, bone_ids));
