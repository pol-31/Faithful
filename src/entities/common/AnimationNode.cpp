...
void AnimationNode::ProcessAnimationKey(
  ...
  if (!keys_.Empty()) {
    glm::mat4 animation_s = glm::scale(glm::mat4(1.0f),
                                       AnimScalingByTime(time));
    glm::mat4 animation_r = glm::mat4_cast(AnimRotationByTime(time));
    glm::mat4 animation_t = glm::translate(glm::mat4(1.0f),
                                           AnimPositionByTime(time));
...
AnimationNode::AnimTransform AnimationNode::AnimTransformByTime(
  ...
  SkipIterator<float> begin(
    &*(keys_.begin() + global_offset + local_size - 1),
    local_size);
  ...
  if ((resultIterator == begin) ||
    ...
    unsigned int offset = std::distance(keys_.get_data(), &*end)
                          - local_size + 1;
    // TODO: we don't need result continiously (every frame) but sometimes only once
    return {offset, offset, 0};
  }
  unsigned int offset_last = std::distance(keys_.get_data(), &*resultIterator)
                             - local_size + 1;
