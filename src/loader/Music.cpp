...
std::array<MusicData, faithful::config::max_active_music_num> music_heap_data_;
...
InstanceInfo MusicManager::Load(std::string&& music_path) {
  auto [sound_id, ref_counter, is_new_id] = Base::AcquireId(music_path);
  if (sound_id == 0) {
    return {"", nullptr, default_music_id_};
  } else if (!is_new_id) {
    return {std::move(music_path), ref_counter, sound_id};
  }
...
bool MusicManager::LoadMusicData(int music_id, const std::string& music_path) {
...
bool MusicManager::CheckStreamErrors(std::ifstream& stream) {
  if(stream.eof()) {
...
void MusicManager::ResetStream(std::ifstream& stream, MusicData& music_data) {
...
ov_callbacks MusicManager::GetOggCallbacks() {
...
ALenum MusicManager::DeduceMusicFormat(const MusicData& music_data) {
